...
export function EditPersonnelDialog({ isOpen, onOpenChange, personnelData }: EditPersonnelDialogProps) {
  ...
  useEffect(() => {
    if (personnelData) {
      form.reset(personnelData);
    }
  }, [personnelData, form]);
  const updatePersonnelMutation = useApiMutation<Personnel, Partial<Personnel>>(
    (updatedPersonnel) => api(`/api/personnel/${personnelData.id}`, { method: 'PUT', body: JSON.stringify(updatedPersonnel) }),
    [['personnel']]
  );
  const onSubmit = (values: z.infer<typeof personnelSchema>) => {
    updatePersonnelMutation.mutate(values, {
      onSuccess: (data) => {
        toast.success(`Personnel "${data.name}" updated successfully!`);
        onOpenChange(false);
      },
      onError: (error) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Edit Personnel: {personnelData.name}</DialogTitle>
          <DialogDescription>
